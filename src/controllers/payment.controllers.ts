...
import { dateFormat, ReturnQueryFromVNPay } from "vnpay";
...
/**
 * @interface OrderData
 * @description Interface định nghĩa cấu trúc dữ liệu đơn hàng cho thanh toán
 */
interface OrderData {
  userId: number;
...
export const createVNPayPayment = async (req: Request, res: Response) => {
  ...
  tempCache.set("orderData", orderData);
  ...
  try {
    ...
    const paymentUrl = await vnpay.buildPaymentUrl({
      ...
      vnp_IpAddr: ((req.headers["x-forwarded-for"] as string) ||
        ...
        req.ip ||
        "127.0.0.1") as string, // Lấy IP của client
      ...
      vnp_Locale: VnpLocale.VN, // Ngôn ngữ tiếng Việt
      vnp_CreateDate: dateFormat(new Date()), // Thời gian tạo giao dịch
    });
...
export const handleVNPayReturn = async (
  req: Request<{}, {}, {}, ReturnQueryFromVNPay>,
  res: Response
) => {
  try {
